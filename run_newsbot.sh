...
echo "📰 Exportando notícias para o portfolio (mix regional BR/IE/UK/US + áudio per-article)..." | tee -a "$LOG_FILE"

# export_portfolio_news.py: news_run.json (que já tem audio) → news.json com
# selecao por cotas de regiao (feeds_config.json campo `region`) + copia MP3s.
$VENV_DIR/bin/python3 "$PROJECT_DIR/export_portfolio_news.py" >> "$LOG_FILE" 2>&1
